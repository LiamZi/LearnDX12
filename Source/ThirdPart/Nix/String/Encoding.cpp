...
// Win32 Api 版本
	...
	{
		int bytes = ::WideCharToMultiByte( CP_OEMCP, 0,(LPCWSTR)unic, (int)len / 2, NULL, 0, NULL, NULL);
		adjust_conv_buffer(bytes+1);
		CONV_BUFF.assign(bytes+1, 0);
		::WideCharToMultiByte(CP_OEMCP, 0, (LPCWSTR)unic, (int)len, (LPSTR)CONV_BUFF.data(), bytes, NULL, NULL);
		*ascii = (char*)CONV_BUFF.data();
		return bytes+1 ;
	}
