...
void Device::createShadersAndLayout(std::unordered_map<std::string, ComPtr<ID3DBlob>> &shaders, 
                                    std::vector<D3D12_INPUT_ELEMENT_DESC> &inputLayout, 
                                    const std::wstring &vertexfilePath,
                                    const std::wstring &fragFilePath)
{
    shaders["standardVS"] = Utils::compileShader(vertexfilePath, nullptr, "vsMain", "vs_5_1");
    shaders["opaquePS"] = Utils::compileShader(fragFilePath, nullptr, "psMain", "ps_5_1");
