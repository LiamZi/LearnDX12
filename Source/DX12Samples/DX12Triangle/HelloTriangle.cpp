...
bool HelloTriangle::initialize(void *_wnd, Nix::IArchive* archive)
{
    ...
    auto file = _archive->open("shaders/triangle/vertexShader.hlsl", 1);
    if(!file) return false;

    rst = D3DCompile(file->constData(), file->size(), 
                    "vertexShader.hlsl", nullptr, nullptr, 
                    "main", "vs_5_0",
    ...
    file = _archive->open("shaders/triangle/fragmentShader.hlsl", 1);
    if(!file) return false;

    rst = D3DCompile(file->constData(), file->size(),
                    "fragmentShader.hlsl", nullptr, nullptr,
                    "main", "ps_5_0",
                    D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION,
